...
import {TFeatures, TMission } from '@/types/types';
import { CiEdit } from 'react-icons/ci';
import Image from 'next/image';
import useAxiosPublic from '@/axios/useAxiosPublic';


const UpdateMissionInfo = ({setUpdateMission}:{setUpdateMission:(value:boolean)=>void}) => {
  const [features,isLoading] = useFeatures();
  const data:TMission = (features as TFeatures)?.mission;
  ...
  const updateMutation = useMutation({
    mutationFn: async ({id,missionData}:{id:string;missionData:Partial<TMission>}) => {
      const response = await apiClient.patch(`/features/${id}`, {mission:missionData});
      return response.data.data;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['features'] });
      toast.success(' Updated Successfully');
      setUpdateMission(false);
    },
    onError: (error: any) => {
  ...
  const handleSubmit = (e: { preventDefault: () => void; target: any; } ) => {
    ...
    const form = e.target;
    const missionData:Partial<TMission> = {
      title: form.title.value,   
      description:form.description.value,
      imageUrl: photoId ? imageUrl : data?.imageUrl,
    };

    updateMutation.mutate({ id:features._id as string, missionData});
    form.reset();
...
export default UpdateMissionInfo;
