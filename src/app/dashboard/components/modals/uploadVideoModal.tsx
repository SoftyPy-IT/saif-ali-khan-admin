...
const UploadVideoModal: React.FC<UploadModalProps> = ({ isOpen, onClose }) => {
  ...
  return isOpen ? (
    ...
    <div className="bg-white  shadow-2xl w-8/12 max-w-lg p-8 relative animate-fade-in-up">
      ...
      <h2 className="text-2xl font-semibold text-gray-800 mb-6 text-center">Upload Video</h2>
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          ...
          >
            ...
            {videoFolder.map((option, index) => (
              <option key={index} value={option.folder} hidden={option.folder==="All Videos"}>
                {option.folder}
              </option>
            ))}
          </select>
