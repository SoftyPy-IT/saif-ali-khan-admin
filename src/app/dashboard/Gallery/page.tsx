...
import AddNewPhoto from '../components/sliders/AddNewPhoto';
import UpdatePhotoInfo from '../components/sliders/UpdatePhotoInfo';
import { TGallery } from '@/types/types';



const Gallery = () => {
const [updateGalleryId,setUpdateGalleryId] = useState("");  
...
const [openModalForUpdate,setOpenModalForUpdate] = useState<boolean>(false);
          ...
          //  getting gallery 
const {data:galleryData={data:[],totalCount:0},isLoading} = useQuery({
  queryKey:["gallery",currentPage],
  queryFn:async()=>{  
    const response = await axiosPublic.get(`/gallery?page=${currentPage}&limit=${limit}`);
    return response.data.data;
  }
})

const {data,totalCount} = galleryData; 
...
<UpdatePhotoInfo galleryId={updateGalleryId} setOpenModalForUpdate={setOpenModalForUpdate}/>
</div>
</div>
     ...
     <h1 className='lg:text-4xl text-xl font-semibold text-orange-500 '>Gallery</h1>
     <button  onClick={()=>setOpenModalForAdd(!openModalForAdd)} className=' active:scale-95 text-xl text-white  p-1 bg-gray-600 hover:bg-slate-800 flex gap-1 items-center pl-3 pr-5'><MdAddBox />Add New Photo</button>
     </div>
  ...
  
  {
  ...
  
    {
      ...
      <Table sx={{ width:'100%',border:"2px solid gray" }} aria-label="simple table">
        <TableHead className='bg-blue-200 border-b-2 border-gray-600'>
          <TableRow>
            ...
            <TableCell align='left' className='text-gray-900 font-semibold'>Photo</TableCell>
      ...
      <TableBody>   
      {data?.map((row:TGallery,idx:number) => (
        <TableRow
          key={idx}
          sx={{ '&:last-child td, &:last-child th': { border: 0 } }}
        >
          <TableCell component="th" scope="row">
           <Image alt='video' src={row.imageUrl} className='w-[60px] h-[60px]' width={60} height={60}/>
          </TableCell>
          <TableCell align="left">{row.title.slice(0,30)}...</TableCell>
          <TableCell align="left">{new Date(row.date).toDateString()}</TableCell>
          <TableCell align="left">{new Date(row.createdAt).toLocaleDateString()}</TableCell>
          <TableCell align="left">
         <div className='flex items-center gap-3'>
         <button  onClick={()=>{setOpenModalForUpdate(!openModalForUpdate);setUpdateGalleryId(row._id)}}  className=' active:scale-95 text-xl text-white  p-1 bg-orange-500 flex gap-1 items-center rounded-full hover:bg-orange-800'><BiSolidEditAlt /></button>
         <button 
          onClick={()=>
              Swal.fire({
                title: "Are you sure?",
