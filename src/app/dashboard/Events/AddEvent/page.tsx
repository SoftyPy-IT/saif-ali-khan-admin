...
import { TEvent, TPhoto } from '@/types/types';
...
const AddNewEvent = () => {
  ...
  const createMutation = useMutation({
    mutationFn: async ({ data }: { data: Partial<TEvent> }) => {
      const response = await apiClient.post(`/events/create-event`, data);
  ...
  const handleSubmit = async (e: { preventDefault: () => void; target: any; }) => {
    ...
    const data: Partial<TEvent> = {
      title, shortDescription, description, date, location, imageUrl
    }
          ...
          <div className='max-w-4xl 2xl:mt-28 px-6 pt-10 flex flex-col  gap-10 justify-between items-center pb-20'>
            ...
            <section className='flex flex-col justify-between lg:flex-row gap-16 w-full h-auto lg:h-[220px]'>
              ...
              <div className=' lg:w-1/2 w-full h-full'>
                {
                  photoId ?
                    ...
                    <label aria-required htmlFor="upload Image">
                      <div onClick={() => toggleDrawer(true)} className='flex items-center flex-col justify-center p-5 border-2 bg-white h-[150px] lg:h-full'>
              ...
              <div className='lg:w-1/2 w-full h-full flex flex-col gap-[20px]'>
                <TextField required value={title}
                  ...
                  name='title' className=' w-full bg-white' id="outlined-basic" label="Title" variant="outlined" />

                <TextField required
                  value={location}
                  onChange={(e) => setLocation(e.target.value)}
                  name='location' className=' w-full bg-white' id="outlined-basic" label="Location" variant="outlined" />
                <label htmlFor="date">
                  ...
                  <input
                    ...
                    type="date" name="date" id="date" className=' w-full bg-white py-3  text-gray-700  px-3' />
                </label>
              </div>
            ...
            <div className='w-full bg-white '>
              ...
              <TextField required value={shortDescription}
                ...
                name='shortDescription' className=' w-full bg-white mb-6' id="outlined-basic" label="Short Description" variant="outlined" />
            ...
            <input type="submit" value="Submit" className=' text-white  bg-orange-600 py-2 px-3 active:scale-95 w-full' />
          </div>
