...
const axiosPublic = useAxiosPublic();
                  ...
                  //  getting events 
const {data:eventsData={data:[],totalCount:0},isLoading} = useQuery({
  queryKey:["events",currentPage],
  queryFn:async()=>{  
    const response = await axiosPublic.get(`/events?page=${currentPage}&limit=${limit}`);
    return response.data.data;
  }
})

           const {data,totalCount} = eventsData; 
  ...
  return (
    <div className='bg-white'>
     ...
     <h1 className='lg:text-4xl text-xl font-semibold text-orange-500 '>Events</h1>
    <Link href={"/dashboard/Events/AddEvent"}>
    <button   className=' active:scale-95 text-xl text-white  p-1 bg-gray-600 hover:bg-slate-800 flex gap-1 items-center pl-3 pr-5'><MdAddBox />Add New Event</button>
    </Link>
     </div>
  ...
  
  {
  ...
  
    {
      ...
      <Table sx={{ width:'100%',border:"2px solid gray" }} aria-label="simple table">
        <TableHead className='bg-blue-200 border-b-2 border-gray-600'>
          <TableRow>
            ...
            <TableCell align='left'  className='text-gray-900 font-semibold'>Location</TableCell>
      ...
      <TableBody>   
      {data?.map((row:TEvent,idx:number) => (
        <TableRow
          key={idx}
        ...
        <Link href={`/dashboard/Events/UpdateEvent/${row._id}`}>
        <button  className=' active:scale-95 text-xl text-white  p-1 bg-orange-500 flex gap-1 items-center rounded-full hover:bg-orange-800'><BiSolidEditAlt /></button>
        </Link>
         <button 
          onClick={()=>
              Swal.fire({
