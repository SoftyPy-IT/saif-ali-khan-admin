...
import AddNewJourney from '../components/sliders/AddNewJourny';
import UpdateJourneyInfo from '../components/sliders/UpdateJourneyInfo';
...
const [updateJourneyToPoliticsId,setJourneyToPoliticsId] = useState("");  
...
const [openModalForUpdate,setOpenModalForUpdate] = useState<boolean>(false);
          ...
          //  getting Journey To Politics
const {data:journeyToPoliticsData={data:[],totalCount:0},isLoading} = useQuery({
  queryKey:["journeyToPolitics",currentPage],
  queryFn:async()=>{  
    const response = await axiosPublic.get(`/journey-to-politics?page=${currentPage}&limit=${limit}`);
    return response.data.data;
  }
})

const {data,totalCount} = journeyToPoliticsData; 

          // delete VoiceOnMedia
          const deleteMutation = useMutation({
            mutationFn: async (id:string) => {
              const response = await apiClient.delete(`/journey-to-politics/${id}`);
              return response.data.data; 
            },
            onSuccess: () => {
...
<UpdateJourneyInfo journeyToPoliticsId={updateJourneyToPoliticsId} setOpenModalForUpdate={setOpenModalForUpdate}/>
</div>
</div>
    ...
    <h1 className='lg:text-4xl text-xl font-semibold text-orange-500 '>Journey To Politics</h1>
    <button  onClick={()=>setOpenModalForAdd(!openModalForAdd)} className=' active:scale-95 text-xl text-white  p-1 bg-gray-600 hover:bg-slate-800 flex gap-1 items-center pl-3 pr-5'><MdAddBox />Add New Journey</button>
    </div>
 ...
 
 {
 ...
 
   {
     ...
     <TableBody>   
     {data?.map((row:TJourneyToPolitics,idx:number) => (
       <TableRow
         key={idx}
       ...
       >
        ...
        <button  onClick={()=>{setOpenModalForUpdate(!openModalForUpdate);setJourneyToPoliticsId(row._id)}}  className=' active:scale-95 text-xl text-white  p-1 bg-orange-500 flex gap-1 items-center rounded-full hover:bg-orange-800'><BiSolidEditAlt /></button>
        <button 
         onClick={()=>
             Swal.fire({
               title: "Are you sure?",
