...
{/* slider for add info  */}
<div className={`transition-transform duration-500 w-full lg:w-4/5  shadow-lg h-full z-10 overflow-y-auto  fixed ${openModalForAdd?'translate-y-0 top-[60px] bg-gray-100':'translate-y-[100%] '}  flex justify-center bg-[url(/Images/bg-image-modal.jpg)]   bg-cover bg-center  `}>
...
<UpdateUserInfo/>
     ...
     <h1 className='lg:text-4xl text-xl font-semibold text-orange-500 '>Manage User</h1>
     <button  onClick={()=>setOpenModalForAdd(!openModalForAdd)} className=' active:scale-95 text-xl text-white  p-1 bg-gray-600 hover:bg-slate-800 flex gap-1 items-center pl-3 pr-5'><MdAddBox />Add New User</button>
     </div>
      ...
      <Table sx={{ width:'100%',border:"2px solid gray" }} aria-label="simple table">
        <TableHead className='bg-blue-200 border-b-2 border-gray-600'>
          <TableRow>
            ...
            <TableCell align='left'  className='text-gray-900 font-semibold'>Name</TableCell>
            <TableCell align='left'  className='text-gray-900 font-semibold'>Email</TableCell>
            <TableCell align='left'  className='text-gray-900 font-semibold'>Current Role</TableCell>
          ...
          {rows.map((row) => (
            ...
            >
              <TableCell component="th" scope="row">
               <Image className='w-[60px] h-[60px]' alt='video' src={row.imageUrl} width={60} height={60}/>
              </TableCell>
             ...
             <button  onClick={()=>setOpenModalForUpdate(!openModalForUpdate)}  className=' active:scale-95 text-xl text-white  p-1 bg-orange-500 flex gap-1 items-center rounded-full hover:bg-orange-800'><BiSolidEditAlt /></button>
             <button className='bg-rose-600 p-1 text-xl rounded-full text-white active:scale-90 hover:bg-red-800'><MdDelete /></button>
             </div>
              </TableCell>      
            </TableRow>
          ))}
